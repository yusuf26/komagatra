...
class Pemasukan_bank extends OperatorController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Pemasukan Bank Tunai';
		...
		$this->data['akun_id'] = $this->pemasukan_bank_m->get_data_akun();
		$this->data['angsuran'] = $this->pemasukan_m->get_angsuran();
		$this->data['setoran'] = $this->pengeluaran_m->get_pengajuan(2,'Sukarela');

		$this->data['isi'] = $this->load->view('pemasukan_bank_list_v', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   = $this->pemasukan_bank_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$nama_kas = $this->pemasukan_m->get_jenis_kas($r->untuk_kas);	
			$nama_akun = $this->pemasukan_m->get_jenis_akun($r->transaksi);	

			$id_txt = 'TKD' . sprintf('%05d', $r->id) . '';
			$jenis = 'Lain-lain';
			if($r->tbl == 'B'){
				$id_txt = 'TBY' . sprintf('%05d', $r->id) . '';
				$jenis = 'Pembayaran Angsuran';
			}

			if($r->tbl == 'C'){
				$id_txt = 'TRD' . sprintf('%05d', $r->id) . '';
				$jenis = 'Pembayaran Angsuran';
			}
			...
			$rows[$i]['ket'] = $r->ket;
			$rows[$i]['jenis'] = $jenis;
			$rows[$i]['jumlah'] = number_format($r->debet);
			$rows[$i]['user'] = $r->user;
	...
	public function create() {
		...
		if($this->pemasukan_bank_m->create()){
	...
	public function update($id=null) {
		...
		if($this->pemasukan_bank_m->update($id)) {
	...
	public function delete() {
		...
		if($this->pemasukan_bank_m->delete($id))
		{
	...
	function cetak_laporan() {
		$pemasukan = $this->pemasukan_bank_m->lap_data_pemasukan();
		if($pemasukan == FALSE) {
		...
		foreach ($pemasukan as $row) {
			$nama_kas = $this->pemasukan_bank_m->get_jenis_kas($row->untuk_kas_id);	
			$nama_akun = $this->pemasukan_bank_m->get_jenis_akun($row->jns_trans);	
	...
	{	
		$data = $this->pemasukan_m->get_data_akun_2();
