...
class Cetak_pengajuan extends OPPController {
	...
	function laporan($type) {
		...
		$data_ajuan = $this->pinjaman_m->get_pengajuan_cetak($type);
	...
	function cetak($id) {
		...
		$title = 'BUKTI PENGAJUAN DANA KREDIT';
		if($row->type == 2){
			$title = 'BUKTI PENGAJUAN DANA SIMPANAN';
		}
		if($row->type == 3){
			$title = 'BUKTI PENGAJUAN DANA PENARIKAN SIMPANAN';
		}
		if($row->type == 4){
			$title = 'BUKTI PENGAJUAN DANA PERUBAHAN SIMPANAN';
		}

		$html .='<div class="h_tengah"><strong> '.$title.' </strong> <br> Ref. '.date('Ymd_His').'</div>
