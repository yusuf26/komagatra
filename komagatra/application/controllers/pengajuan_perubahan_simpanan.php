...
class Pengajuan_perubahan_simpanan extends OperatorController {
	...
	public function index() {
		$this->data['judul_browser'] = 'Pengajuan Perubahan Simpanan';
		$this->data['judul_utama'] = 'Pengajuan Perubahan Simpanan';
		$this->data['judul_sub'] = 'Pengajuan Perubahan Simpanan';
		...
		$this->data['isi'] = $this->load->view('perubahan_simpanan_list_v', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   = $this->simpanan_m->get_data_perubahan_simpanan($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
	...
	public function tambah()
	{
		...
		$this->load->model('simpanan_m');
		$this->load->model('general_m');
		$this->data['judul_browser'] = 'Tambah Pengajuan Perubahan Simpanan';
		$this->data['judul_utama'] = 'Pengajuan Perubahan Simpanan';
		$this->data['judul_sub'] = 'Tambah Pengajuan Perubahan Simpanan';
		...
		$this->data['isi'] = $this->load->view('pengajuan_perubahan_simpanan_tambah_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);	
	}
	...
	public function proses_tambah()
	{
		...
		if($this->db->insert('tbl_pengajuan',$data)){
			...
			$this->notif_m->created_notif(2,$ajuan_id,$pesan,$anggota,4);
	...
	{	
		...
		$jenis_id = $this->db->select('jns_simpan')->get_where('jns_simpan',array('id'=>$jenis))->row_array()['jns_simpan'];
		$result = $this->member_m->get_simpanan_bulan_by_anggota($anggota_id,$jenis_id);
	...
	public function setuju()
	{
		...
		if(!empty($data)){
			foreach ($data as $key => $row) {
				...
				if(strpos($row['jenis_id_txt'] , 'Sukarela') !== false ){
					$jenis_id = 'simpanan_sukarela';
				}else if(strpos($row['jenis_id_txt'] , 'Wajib') !== false ){
					$jenis_id = 'simpanan_wajib';
				}else {
					$jenis_id = 'simpanan_pokok';
				}
				
				$this->db->update('tbl_anggota',array($jenis_id=>str_replace(',', '', $row['jumlah'])),array('id'=>$row['anggota_id']));
