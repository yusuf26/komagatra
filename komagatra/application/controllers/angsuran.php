...
class Angsuran extends OperatorController {
	...
	public function index($master_id = NULL) {
		...
		$ags_per_bulan = $row_pinjam->ags_per_bulan;
		$pokok_angsuran = $row_pinjam->pokok_angsuran;
		$biaya_bunga = $row_pinjam->biaya_adm + $row_pinjam->bunga_pinjaman;

		$jenis_pinjaman = $this->db->get_where('jns_pinjaman',array('jns_pinjaman'=>$row_pinjam->jenis))->row_array();
			
		if($jenis_pinjaman['anuitas'] == 1){
			$pokok_angsuran = $this->finance->ppmt(0.10/12, $row_pinjam->bln_sudah_angsur + 1, $row_pinjam->lama_angsuran, -$row_pinjam->jumlah, 0, false);
			$biaya_bunga = $this->finance->ipmt(0.10/12, $row_pinjam->bln_sudah_angsur + 1, $row_pinjam->lama_angsuran, -$row_pinjam->jumlah, 0, false);

			$ags_per_bulan = $pokok_angsuran + $biaya_bunga; 
		}
		$this->data['ags_per_bulan'] = $ags_per_bulan; 
		$this->data['pokok_angsuran'] = $pokok_angsuran; 
		$this->data['biaya_bunga'] = $biaya_bunga; 
	...
	function ajax_list($id = NULL) {
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['jumlah_bayar'] = number_format(nsi_round($r->jumlah_bayar + $r->simpan_wajib));
	...
	function get_ags_ke($master_id) {
		...
		if($jenis_pinjaman['anuitas'] == 1){
			...
			$row_pinjam->ags_per_bulan = $pokok_angsuran + $biaya_bunga; 
		}
		...
		$total_bunga = $row_pinjam->bunga_pinjaman * $sisa_ags_det;
