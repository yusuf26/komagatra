...
class Member extends CI_Controller {
	...
	public function index() {
		...
		if ($this->session->userdata('login') == TRUE && $this->session->userdata('level') == 'member') {
			...
			if($this->member->validasi()) {
				// echo sha1('nsi' . $this->input->post('pass_word'));
	...
	function simulasi() {
		...
		for ($i=1; $i <= $lama_ags; $i++) { 
			...
			if($get_data_p['anuitas'] == 0){
				$odat['biaya_adm'] = number_format($biaya_admin,0,'','.');
				$odat['biaya_bunga'] = number_format($biaya_bunga,0,'','.');
				$odat['jumlah_ags'] = number_format($angsuran_pokok + $biaya_admin + $biaya_bunga,0,'','.');
			}else {
				...
				$odat['pokok_pinj'] =  number_format($pokok_pinj,0,'','.');
				$odat['cicil'] =  number_format($cicil,0,'','.');
				$odat['margin'] =  number_format($margin,0,'','.');
				$odat['jumlah_ags'] = number_format($cicil + $margin,0,'','.');
				$pokok_pinj = $pokok_pinj - $cicil;
				$odat['saldo_pinj'] = number_format($pokok_pinj,0,'','.');
			...
			$odat['tgl_tempo'] = jin_date_ina($tgl_tempo,'full',false,true);
			$out[] = $odat;
		}
		//var_dump($out);
		if(!empty($out)) {
			...
			echo '		<th style="text-align: center;">Bulan Tempo</th>';
			if($get_data_p['anuitas'] == 0){
				...
				echo '		<th style="text-align: center;">Pokok Pinjaman</th>';
				echo '		<th style="text-align: center;">Cicilan Pokok Pinjaman</th>';
				echo '		<th style="text-align: center;">Margin</th>';
				echo '		<th style="text-align: center;">Angsuran Per Bulan</th>';
				echo '		<th style="text-align: center;">Saldo Pokok Pinjaman</th>';
			}
			echo '	</tr>';
			$no = 1;
			foreach ($out as $val) {
				...
				if($get_data_p['anuitas'] == 0){
					...
					echo '	<td style="text-align: center;">'.$val['pokok_pinj'].'</td>';
					echo '	<td style="text-align: center;">'.$val['cicil'].'</td>';
					echo '	<td style="text-align: center;">'.$val['margin'].'</td>';
					echo '	<td style="text-align: center;">'.$val['jumlah_ags'].'</td>';
					echo '	<td style="text-align: center;">'.$val['saldo_pinj'].'</td>';
				}
				echo '</tr>';
				$no++;
			}
	...
	public function view() {
		...
		$this->data['data_pengajuan'] =  $this->member->get_last_pengajuan(1);
		$this->data['data_pengajuan_simpanan'] =  $this->member->get_last_pengajuan(2);
	...
	public function pengajuan_simpanan() {
		...
		$this->load->view('themes/member_pengajuan_simpanan_v', $this->data);		
	}

	public function pengajuan_penarikan_simpanan() {
		$this->_cek_login();
		$this->load->model('pinjaman_m');
		$this->load->helper('fungsi');
		//editable
		...
		$this->load->view('themes/member_pengajuan_penarikan_simpanan_v', $this->data);		
	}

	public function pengajuan_perubahan_simpanan() {
		$this->_cek_login();
		$this->load->model('pinjaman_m');
		$this->load->helper('fungsi');
		//editable
		...
		$this->load->view('themes/member_pengajuan_perubahan_simpanan_v', $this->data);		
	}
	...
	public function ajax_pengajuan_simpanan() {
		$this->load->model('member_m');
		$out = $this->member_m->get_pengajuan_simpanan();
		header('Content-Type: application/json');
		echo json_encode($out);
		exit();		
	}

	public function ajax_pengajuan_penarikan_simpanan() {
		$this->load->model('member_m');
		$out = $this->member_m->get_pengajuan_penarikan_simpanan();
		header('Content-Type: application/json');
		echo json_encode($out);
		exit();		
	}

	public function ajax_pengajuan_perubahan_simpanan() {
		$this->load->model('member_m');
		$out = $this->member_m->get_pengajuan_perubahan_simpanan();
		header('Content-Type: application/json');
		echo json_encode($out);
		exit();		
	}
	...
	public function pengajuan_baru() {
		...
		$this->data['jenis_pinjaman'] = $this->pinjaman_m->get_data_pinjaman();
		$this->data['tersimpan'] = '';
	...
	public function pengajuan_simpanan_baru() {
		$this->_cek_login();
		...
		if ($this->input->post('submit')) {
			if($this->member->validasi_pengajuan()) {
				$pengajuan_simpan = $this->member->pengajuan_simpanan_simpan();
				if($pengajuan_simpan) {
					$this->session->set_flashdata('ajuan_baru', 'Y');
					redirect('member/pengajuan_simpanan');
				} else {
					$this->data['tersimpan'] = 'N';
				}
			}
		...
		$this->load->helper('fungsi');
		$this->load->view('themes/member_pengajuan_simpanan_baru_v', $this->data);		
	}

	public function pengajuan_penarikan_simpanan_baru() {
		$this->_cek_login();
		...
		if ($this->input->post('submit')) {
			if($this->member->validasi_pengajuan()) {
				$pengajuan_simpan = $this->member->pengajuan_penarikan_simpanan_simpan();
				if($pengajuan_simpan) {
					$this->session->set_flashdata('ajuan_baru', 'Y');
					redirect('member/pengajuan_penarikan_simpanan');
				} else {
					$this->data['tersimpan'] = 'N';
				}
			}
		...
		$this->load->helper('fungsi');
		$this->load->view('themes/member_pengajuan_penarikan_simpanan_baru_v', $this->data);		
	}

	public function pengajuan_perubahan_simpanan_baru() {
		$this->_cek_login();
		...
		if ($this->input->post('submit')) {
			if($this->member->validasi_pengajuan()) {
				$pengajuan_simpan = $this->member->pengajuan_perubahan_simpanan_simpan();
				if($pengajuan_simpan) {
					$this->session->set_flashdata('ajuan_baru', 'Y');
					redirect('member/pengajuan_perubahan_simpanan');
				} else {
					$this->data['tersimpan'] = 'N';
				}
			}
		...
		$this->load->helper('fungsi');
		$this->load->view('themes/member_pengajuan_perubahan_simpanan_baru_v', $this->data);		
	}
	...
	{	
		$jenis = $this->input->post('jenis');
		$anggota_id = $this->session->userdata('u_name');
		$out = array('output'=>false);

		$jenis_id = $this->db->select('id')->get_where('jns_simpan',array('jns_simpan'=>$jenis))->row_array()['id'];

		$result = $this->member->get_simpanan_by_anggota($anggota_id,$jenis_id);
	...
	public function get_simpanan_bulan_by_anggota()
	{	
		$jenis = $this->input->post('jenis');
		$anggota_id = $this->session->userdata('u_name');
		$out = array('output'=>false);

		$result = $this->member->get_simpanan_bulan_by_anggota($anggota_id,$jenis);
	...
	public function notif() {
		...
		$this->db->update('tbl_notif',array('baca'=>1,'baca_by'=>$this->session->userdata('u_name'),'baca_at'=>date('Y-m-d H:i:s')),array('baca'=>0,'type'=>2,'anggota_id'=>$this->session->userdata('u_name')));
		$this->load->helper('fungsi');
		$this->load->view('themes/member_notif_v', $this->data);
	}	

	public function ajax_notif() {
		$this->load->model('member_m');
		$out = $this->member_m->get_notif($this->uri->segment(3));
		header('Content-Type: application/json');
		echo json_encode($out);
		exit();		
	}
	...
	public function check_urutan()
	{
		$jenis = $this->input->post('jenis');
		$urutan = array('output'=>1);
		$check_urutan = $this->general_m->check_urutan($jenis);
		$urutan = array('output'=> $check_urutan);
