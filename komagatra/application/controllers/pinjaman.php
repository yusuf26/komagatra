...
class Pinjaman extends OperatorController {
	...
	function ajax_list() {
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['anggota_id_txt'] = $anggota->identitas.' <br>'.$anggota->nama.' <br>'.$this->general_m->get_departement($anggota->departement);
			...
			$rows[$i]['tipe'] = $r->type;
			$rows[$i]['tipe_txt'] = ($r->type == 1) ? 'Barang' : 'Dana';

			$label_barang = 'ID Ajuan';
			$nama_barang = false;
			if($r->barang_id){
				$nama_barang = $barang->nm_barang;
				$label_barang = 'Nama Barang';
			}
			if($r->ajuan_id){
				$nama_barang = $r->ajuan_id;
			}

			$rows[$i]['hitungan'] = '<table width="100%">
						<tr>
							<td width="100px" align="left">'.$label_barang.'</td> 
							<td width="10px" align="center"> : </td>
							<td width="75px" align="right">'.$nama_barang.'</td>
						</tr>
						<tr>
			...
			$rows[$i]['tagihan'] = '<table style="width:100%;">
						<tr>
