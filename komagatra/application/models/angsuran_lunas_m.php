...
class Angsuran_lunas_m extends CI_Model {
		public function __construct(){
		parent::__construct();
	}

	function get_data_kas($type=false) {
		$this->db->select('*');
	...
	function get_data_transaksi_ajax($offset, $limit, array $q = array(), $sort, $order, $id) {
		...
		if(is_array($q)) {
			...
			if(isset($q['pelunasan'])) {
				$sql .=" AND (ket_bayar = 'Pelunasan' OR ket_bayar = 'Bayar Denda') ";
			} else {
	...
	public function create() {
		...
		$data = array(			
					...
					'angsuran_ke'	=>	$ags_ke,
					'jumlah_bayar'	=>	$jumlah_bayar - $total_bunga,
					'simpan_wajib' 	=> $total_bunga,
					'ket_bayar'		=>	'Pelunasan',
		...
		// Insert To Data Pinjaman Wajib Anggota
		$simp_anggota = array(
