			...
			<?php
				...
				if($data_pengajuan) {
					...
					if($data_pengajuan->status == 1) {
						$status = '<span class="text-success"><i class="fa fa-check-circle"></i> Disetujui - Perkiraan Tanggal Cair: '.jin_date_ina($data_pengajuan->tgl_cair, 'full').'</span>';
					}
				...
				if($data_pengajuan_simpanan) {
					$status = '';
					if($data_pengajuan_simpanan->status == 0) {
						$status = '<span class="text-primary"><i class="fa fa-question-circle"></i> Menunggu Konfirmasi</span>';
					}
					if($data_pengajuan_simpanan->status == 1) {
						$status = '<span class="text-success"><i class="fa fa-check-circle"></i> Disetujui - Pada Tanggal : '.jin_date_ina($data_pengajuan_simpanan->tgl_cair, 'full').'</span>';
					}

					if($data_pengajuan_simpanan->status == 2) {
						$status = '<span class="text-danger"><i class="fa fa-times-circle"></i> Ditolak</span>';
					}
					if($data_pengajuan_simpanan->status == 3) {
						$status = '<span class="text-success"><i class="fa fa-rocket"></i> Terlaksana</span>';
					}
					if($data_pengajuan_simpanan->status == 4) {
						$status = '<span class="text-warning"><i class="fa fa-trash-o"></i> Batal</span>';
					}
					$isi .= '<div class="alert alert-warning">Pengajuan Simpanan Mutakhir: <strong>'.jin_date_ina($data_pengajuan_simpanan->tgl_update, 'full', true).'</strong> Nominal: <strong>'.number_format($data_pengajuan_simpanan->nominal).'</strong> Status: <strong>'.$status.'</strong></div>';
				} else {
					$isi .= '<div class="alert alert-warning">Belum ada Pengajuan Simpanan</div>';
				}
