...
url="<?php echo site_url('notifikasi/ajax_list'); ?>" 
pagination="true" rownumbers="true" 
fitColumns="true" collapsible="true"
sortName="created_at" sortOrder="desc"
toolbar="#tb"
striped="true">
<thead>
	<tr>
		...
		<th data-options="field:'tgl',halign:'center', align:'center'" hidden="true">Tanggal</th>
		<th data-options="field:'ajuan_id', width:'5', halign:'center', align:'center'">Ajuan ID</th>
		<th data-options="field:'tgl_txt', width:'5', halign:'center', align:'center'">Tanggal</th>
		<th data-options="field:'anggota', width:'5', halign:'center', align:'center'">Anggota</th>
		<th data-options="field:'pesan', width:'25', halign:'center', align:'left'">Pesan</th>
...
<div id="tb" style="height: 35px;">
	<div style="vertical-align: middle; display: inline; padding-top: 15px;">
		<!-- <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="tolak()">Baca</a>
...
function setuju(){
	...
	if(row.length > 0){
		...
		if($.inArray('1', check) == 0|| $.inArray('2', check) == 0){
			...
			$('#dialog-form').dialog('open').dialog('setTitle','Data Pengajuan Penarikan Simpanan');
			$('#form').form('clear');
			...
			url = '<?php echo site_url('pengajuan_penarikan_simpanan/setuju'); ?>';
		}
	}else {
...
function tolak(){
	...
	if(row.length > 0){
		...
		if($.inArray('1', check) == 0|| $.inArray('2', check) == 0){
			...
			$('#dialog-tolak').dialog('open').dialog('setTitle','Data Pengajuan Penarikan Simpanan');
			$('#form').form('clear');
		}
	}else {
		$.messager.alert('Warning','Maaf, Silahkan Pilih Pengajuan terlebih dahulu.');
...
function save_tolak() {
	...
	if (isValid) {
		...
		$.ajax({
			type	: "POST",
			url: '<?php echo site_url('pengajuan_penarikan_simpanan/tolak');?>',
		...
		if($.inArray('1', check) == 0){
			...
			$.messager.confirm('Konfirmasi','Apakah Anda akan menghapus data ini ?',function(r){  
				if (r){  
					$.ajax({
						type	: "POST",
						url		: "<?php echo site_url('pengajuan_penarikan_simpanan/delete'); ?>",
						data	: {data:row},
						success	: function(result){
