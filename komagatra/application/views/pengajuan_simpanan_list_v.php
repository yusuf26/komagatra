...
<style type="text/css">
	...
	<?php
	echo $this->general_m->get_kode_transaksi('C');
	?>
	<div class="row">
		<div class="box box-primary">
			<div class="box-body" style="min-height: 500px;">
				<div>
					<p style="text-align:center; font-size: 15pt; font-weight: bold;"> Data Pengajuan Simpanan</p>
				</div>	
				<?php
				// print_r($this->general_m-> get_urutan(2));
				// echo 
				?>
				...
				<table 
					...
					data-toggle="table"
					data-id-field="id"
					data-url="<?php echo site_url('pengajuan_simpanan/ajax_pengajuan'); ?>" 
					...
					<thead>
						<tr>
							...
							<th data-field="tgl_input" data-sortable="true" data-valign="middle" data-align="center" data-halign="center" data-formatter="tgl_input_ft">Bulan<br>Pengajuan</th>
							...
							<th data-field="urutan" data-sortable="true" data-align="left" data-halign="center" data-valign="middle" >Urutan</th>
...
<script type="text/javascript">
	...
	function aksi_ft(value, row, index) {
		...
		var link_bayar = '<a data-data_aksi="Bayar" data-data_id="'+row.id+'" data-jenis="'+row.jenis+'" class="a_diterima btn btn-sm btn-primary" href="javascript:void(0);"><i class="fa fa-money"></i> Bayar</a>';
		if(row.status == 1 && row.lama_ags > 0) {
			nsi_out += link_bayar;
		}
		
		<?php if($this->session->userdata('level') != 'operator') { ?>
			var link_diterima = '<a data-data_aksi="Setuju" data-data_id="'+row.id+'" data-jenis="'+row.jenis+'" class="a_diterima btn btn-sm btn-success" href="javascript:void(0);"><i class="fa fa-check-circle"></i> Setujui</a>';
	...
	$(function() {
		...
		$table.on('click', '.a_diterima, .a_ditolak, .a_dipending, .a_hapus, .a_dilaksanakan, .a_belum, .a_dibatal', function(event) {
			...
			if(data_aksi == 'Setuju') {
				...
				fm_tgl_cair = '<div id="div_tgl_cair" class="form-group">'+
										...
										// 	'<select id="kas" name="kas_id" class="form-control" required="true">'+
										// 		'<?php foreach ($kas_id as $row) {echo '<option value="'.$row->id.'">'.$row->nama.'</option>';}?></select>'+
										// '</div>'+
			...
			if(data_aksi == 'Bayar') {
				...
				fm_tgl_cair = '<div id="div_tgl_cair" class="form-group">'+
										...
										'<div class="input-group">'+
											'<input type="number" class="form-control text-center" name="angsuran_ke" id="angsuran_ke" readonly="" style="height:20px;width:30px;"/>'+
										'</div>'+
										'<br />'+
										'<label>Jumlah Simpanan :</label>'+
										'<div class="input-group">'+
											'<input type="text" class="form-control" name="jumlah_simp" id="jumlah_simp" readonly="" style="height:20px;" />'+
										'</div>'+
										'<br />'+
										'<label>Debit :</label>'+
										'<div class="input-group">'+
		...
		$('#link_konfirmasi').click(function(event) {
			...
			$.ajax({
				url: '<?php echo site_url('pengajuan_simpanan/aksi'); ?>',
				type: 'POST',
				dataType: 'json',
	...
	function cetak_laporan () {
		...
		var win = window.open('<?php echo site_url("cetak_pengajuan/laporan/2/?fr_jenis=' + fr_jenis + '&fr_status=' + fr_status + '&fr_bulan=' + fr_bulan + '&tgl_dari=' + tgl_dari + '&tgl_sampai=' + tgl_sampai + '"); ?>');
		if (win) {
			win.focus();
		} else {
