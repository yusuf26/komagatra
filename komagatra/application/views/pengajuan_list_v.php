	...
	<div class="row">
		<div class="box box-primary">
			<div class="box-body" style="min-height: 500px;">
				<div>
					<p style="text-align:center; font-size: 15pt; font-weight: bold;"> Data Pengajuan Pinjaman</p>
				</div>
...
<script type="text/javascript">
	...
	function status_ft(value, row, index) {
		...
		if(value == 1) {
			...
			nsi_out += '<br>Perkiraan Cair: ' + row.tgl_cair_txt;
		}
		if(value == 2) {
	...
	$(function() {
		...
		$table.on('click', '.a_diterima, .a_ditolak, .a_dipending, .a_hapus, .a_dilaksanakan, .a_belum, .a_dibatal', function(event) {
			...
			if(data_aksi == 'Setuju') {
				...
				fm_tgl_cair = '<div id="div_tgl_cair" class="form-group">'+
										...
										// '<label>Bunga:</label>'+
										// '<div class="input-group">'+
										// 	'<input type="hidden" name="bunga" id="bunga" value="<?= $bunga;?>" readonly="readonly" />'+
										// 	'<input type="text" id="bunga_txt" class="form-control" value="<?= $bunga;?> %" readonly="true" />'+
										// '</div>'+
										// '<br />'+
										// '<label>Biaya Admin:</label>'+
										// '<div class="input-group">'+
										// 	'<input type="text" id="biaya_adm" name="biaya_adm" class="form-control" value="<?= $biaya_adm;?>" readonly="true" />'+
										// '</div>'+
										// '<br />'+
										// '<label>Debit :</label>'+
										// '<div class="input-group">'+
		...
		$('#link_konfirmasi').click(function(event) {
			...
			$.ajax({
				...
				type: 'POST',
				dataType: 'html',
				data: {id: data_id, aksi: data_aksi, tgl_cair: data_tgl_cair, alasan: data_alasan, bunga:bunga, biaya_adm:biaya_adm, kas_id:kas_id},
			})
			.done(function(data) {
				if(data == 'OK') {
	...
	function cetak_laporan () {
		...
		var win = window.open('<?php echo site_url("cetak_pengajuan/laporan/1/?fr_jenis=' + fr_jenis + '&fr_status=' + fr_status + '&fr_bulan=' + fr_bulan + '&tgl_dari=' + tgl_dari + '&tgl_sampai=' + tgl_sampai + '"); ?>');
		if (win) {
			win.focus();
		} else {
