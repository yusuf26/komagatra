			...
			<tr>
				...
				<td>
					<form id="fmCari">
						...
						<select id="cari_akun" name="cari_akun[]" class="" multiple="" style="width:200px;">
							<?php
							foreach ($cari_akun as $key => $ck) {
								?>
								<option value="<?php echo $ck->id?>"><?php echo $ck->kd_aktiva.' - '.$ck->jns_trans;?></option>
								<?php
							}
							?>
						</select>
...
$(document).ready(function() {
	...
	$('#cari_akun').select2('val',<?php echo json_encode($get_akun);?>);

	$('#cari_akun').change(function(){
		doSearch();
	});
}); // ready
