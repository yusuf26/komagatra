...
<?php 
	...
	if(isset($_REQUEST['cari_bank'])){
		$get_akun = $_REQUEST['cari_bank'];
	}
?>

<div class="box box-solid box-primary">
	<div class="box-header">
		<h3 class="box-title">Cetak Data Transaksi Bank</h3>
			...
			<form id="fmCari" method="GET">
				...
				<table>
					<tr>
						...
						<td>
							<select id="cari_bank" name="cari_bank[]" multiple="">
								<?php
								foreach ($kas_id as $key => $row) {
									?>
...
<div class="box-body">
<p></p>
	<p style="text-align:center; font-size: 15pt; font-weight: bold;"> Laporan Transaksi Bank Periode <?php echo $tgl_periode_txt; ?></p>
...
<script type="text/javascript">
	...
	$('#cari_bank').select2('val',<?php echo json_encode($get_akun);?>);
...
function clearSearch(){
	window.location.href = '<?php echo site_url("lap_trans_kas/bank"); ?>';
}

$('#cari_bank').change(function(){
	doSearch();
});
...
function doSearch() {
	...
	$('#fmCari').attr('action', '<?php echo site_url('lap_trans_kas/bank'); ?>');
	$('#fmCari').submit();	
}
...
function cetak () {
	...
	var win = window.open('<?php echo site_url("lap_trans_kas/cetak/bank/?tgl_dari=' + tgl_dari + '&tgl_samp=' + tgl_samp + '"); ?>');
	if (win) {
		win.focus();
	} else {
