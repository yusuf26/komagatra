	...
	foreach ($data_kas as $row) {
		...
		echo '
			<tr bgcolor='.$warna.'>
					...
					<td class="h_tengah" style="vertical-align:middle"> '.$row->kd_transaksi.'</td>
...
function doSearch() {
	...
	$('#fmCari').attr('action', '<?php echo site_url('lap_trans_kas/kas'); ?>');
	$('#fmCari').submit();	
}
...
function cetak () {
	...
	var win = window.open('<?php echo site_url("lap_trans_kas/cetak/kas/?tgl_dari=' + tgl_dari + '&tgl_samp=' + tgl_samp + '"); ?>');
	if (win) {
		win.focus();
	} else {
