...
title="Data Transaksi Pengeluaran Bank" 
style="width:auto; height: auto;" 
url="<?php echo site_url('pengeluaran_bank/ajax_list'); ?>" 
...
function create(){
	...
	url = '<?php echo site_url('pengeluaran_bank/create'); ?>';
}
...
function update(){
	...
	if(row){
		...
		url = '<?php echo site_url('pengeluaran_bank/update'); ?>/' + row.id;
		$('#jumlah ~ span input').keyup(function(){
			var val_jumlah = $(this).val();
			$('#jumlah').numberbox('setValue', number_format(val_jumlah));
		});
		...
		if(row.status_id == 1){
			...
			$.messager.confirm('Konfirmasi','Apakah Anda akan menghapus data kode transaksi : <code>' + row.id_txt + '</code> ?',function(r){  
				if (r){  
					$.ajax({
						type	: "POST",
						url		: "<?php echo site_url('pengeluaran_bank/delete'); ?>",
						data	: 'id='+row.id,
						success	: function(result){
							var result = eval('('+result+')');
							$.messager.show({
...
function cetak () {
	...
	if(row){
		...
		var win = window.open('<?php echo site_url("cetak/cetak_laporan/0/bank/?kode_transaksi=' + kode_transaksi + '&tgl_dari=' + tgl_dari + '&tgl_sampai=' + tgl_sampai + '"); ?>');
		if (win) {
			win.focus();
		} else {
		...
		$.ajax({
			...
			success	: function(result){
				if(result.ok) {
					...
					var win = window.open('<?php echo site_url("cetak/cetak_laporan/0/bank/'+row.tbl+'?kode_transaksi=' + kode_transaksi+'&tgl_dari=&tgl_sampai="); ?>','_blank');
					if (win) {
						win.focus();
					} else {
						alert('Popup jangan di block');
